...
<div id="app">
    <section class="todo-wrapper">
      ...
      <div v-if="pending.length > 0">
        <p class="status busy">Tienes {{ pending.length }} tareas pendiente<span v-if="pending.length > 1">s</span></p>
        <transition-group name="todo-item" tag="ul" class="todo-list">
          <Tarea 
             v-bind:todoList="todoList" 
             v-for="(item) in pending" 
             v-bind:key="item.title" 
             v-bind:item='item'></Tarea>
        </transition-group>  
      </div> 
...
export default {
  ...
  computed: {
    pending: function() {
      return this.todoList.filter(function(item) {
        return !item.done; // item falso
      })
    },
    completed: function() {
      return this.todoList.filter(function(item) {
        return item.done; // item verdadero
      }); 
    },
  ...
  methods:{
    toggleShowComplete() {
      this.showComplete = !this.showComplete; // true
    },
    addItem() {
      if (this.new_todo) {
